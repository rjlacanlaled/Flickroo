...
{
    return [self URLForQuery:@"https://api.flickr.com/services/rest/?method=flickr.places.getTopPlacesList&place_type_id=8"];
}
...
return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.photos.getInfo&photo_id=%@", photoID]];

}

+ (NSURL *)URLforPhotosAtLat:(NSNumber*)latitude andLong:(NSNumber*)longitude{   return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.photos.search&license=1,2,4,7&has_geo=1&lat=%f&lon=%f&extras=original_format,description,geo,date_upload,owner_name",[latitude floatValue],[longitude floatValue]]];   }


+ (NSURL *)URLforLocationDetailsForPhotoID:(NSString *)photoID {
    return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.photos.geo.getLocation&photo_id=%@", photoID]];
}

+ (NSURL *)URLforDetailedInfoForPhotoID: (NSString *)photoID {
    return [self URLForQuery:[NSString stringWithFormat:
                              @"https://www.flickr.com/services/rest/?method=flickr.photos.getInfo&photo_id=%@", photoID
                              ]];
}
@end
